...
enum DeteksiState { none, loading }

class DeteksiViewModel with ChangeNotifier {
  File? img;
  List result = [];
  String? predLabel;
  XFile? imagePicked;
  ...
  DeteksiState state = DeteksiState.none;
  ...
  Future btnAction(source) async {
    ...
    notifyListeners();
    changeState(DeteksiState.none);
  }

  changeState(DeteksiState s) {
    state = s;
    notifyListeners();
  }
}
